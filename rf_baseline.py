...
for r in range(10):
	...
	print("num estimators = {}".format(n_e))
	print("training for all samples")
	rf = RandomForestClassifier(n_estimators=n_e,  bootstrap = True, max_features = 'sqrt')
	rf.fit(train_X_all, train_label_all)
	print("scoring for all samples")
	s_all = rf.score(test_X_all, test_label_all)
	print(s_all)
	rf_all.append(s_all)

	print("training for cancer samples")
	rf = RandomForestClassifier(n_estimators=n_e,  bootstrap = True, max_features = 'sqrt')
	rf.fit(train_X_cancer, train_label_cancer)
	print("scoring for cancer samples")
	s_can = rf.score(test_X_cancer, test_label_cancer)
	print(s_can)
	rf_can.append(s_can)
